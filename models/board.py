...
class Point:
    ...
    def __post_init__(self):
        if self.row < 0 or self.row >= BOARD_HEIGHT:
            raise ValueError(f"Row must be between 0 and {BOARD_HEIGHT - 1}")
        if self.column < 0 or self.column >= BOARD_WIDTH:
...
class Board:
    ...
    def can_add_ship(self, ship: Ship, position: Point) -> bool:
        ...
        if position.row + height > BOARD_HEIGHT or position.column + width > BOARD_WIDTH:
            return False
        
        start_row = max(position.row - 1, 0)
        end_row = min(position.row + height + 1, BOARD_HEIGHT)
        start_col = max(position.column - 1, 0)
        end_col = min(position.column + width + 1, BOARD_WIDTH)
